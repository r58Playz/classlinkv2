...
export async function uiHelper(req, res, options, handler) {
  ...
  // start session so classlink doesn't get mad
  const startSessionResp = await postNoBodyWithBearer("https://applications.apis.classlink.com/v1/startSession", data.cookies.t).then(r=>r.text());
  if(startSessionResp==="Token invalid or expired") {
    cookies.set("cl-token");
    cookies.set("cl-gws");
    cookies.set("cl-code");
    return {redirect: {permanent: false, destination: "/"}};
  }
  try {
      const ssrJson = JSON.parse(startSessionResp)
      if(ssrJson.message === "Invalid or expired access token" || ssrJson.message === "Invalid OA2 Token") {
        cookies.set("cl-token");
        cookies.set("cl-gws");
        cookies.set("cl-code");
        return {redirect: {permanent: false, destination: "/"}};
  ...
  if(startSessionResp!=='"Session Started"') throw new Error(startSessionResp);

  const userData = await fetchWithBearer("https://nodeapi.classlink.com/v2/my/info", t).then(r=>r.text());
  data.userData = JSON.parse(userData);
  
  const handlerData = await handler(req, res, data);

  // end session so classlink doesn't get mad
  const endSessionResp = await postNoBodyWithBearer("https://applications.apis.classlink.com/v1/endSession", data.cookies.t).then(r=>r.text());
  if(endSessionResp!=='"Session Ended"') throw new Error(endSessionResp);
